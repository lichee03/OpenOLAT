...
/**
 * Validates that the user is not already enrolled in the group.
 * 
...
public class AlreadyEnrolledRule implements PrerequisiteRule {
    ...
    public EnrollmentEligibility validate(Identity identity, BusinessGroup group, 
        ...
        boolean isParticipant = businessGroupService.hasRoles(identity, group, 
                BusinessGroupService.GROUP_PARTICIPANT);
        
        if (isParticipant) {
            return EnrollmentEligibility.notEligible(
                    String.format("You are already enrolled in group '%s'", group.getName()));
        }
    ...
    public int getPriority() {
        return 15; // Check this early to avoid unnecessary processing
    }
